...
def freshdesk_api_key() -> str:
    ...
    Tests monkeypatch env vars after import, so we must not snapshot values at import time.
    """
...
class CompanyCreate(BaseModel):
    ...
    renewal_date: Optional[date] = Field(
        None, description="Renewal date for the company's contract (YYYY-MM-DD)"
    )
    industry: Optional[str] = Field(None, description="Industry to which the company belongs")
    custom_fields: Optional[Dict[str, Any]] = Field(
        None, description="Key-value pairs of custom fields for the company"
    )
...
class CompanyFieldCreate(BaseModel):
    ...
    required_for_agents: Optional[bool] = Field(
        None, description="Set to true if the field is mandatory for agents"
    )
    ...
    async with httpx.AsyncClient() as client:
        try:
            if avatar_path:
                ...
                # Ensure we close the file handle (long-running server safety).
                with open(avatar_path, "rb") as avatar_f:
                    files = {
                        "avatar": (
                    ...
                    response = await client.post(
                        url, headers=headers, data=contact_data, files=files
                    )
            else:
...
@mcp.tool()
async def list_roles() -> Union[List[Dict[str, Any]], Dict[str, Any]]:
...
@mcp.tool()
async def list_products() -> Union[List[Dict[str, Any]], Dict[str, Any]]:
